...
const ACTIVE_COLOR: string = theme.color.primary
const INACTIVE_COLOR: string = theme.color.lightGray
const CURRENT_COLOR: string = `${theme.color.primary}`
...
export function Pagination<T extends PaginationProps>(props: T & ViewProps): React.ReactElement {
    const {
        activeIndex,
        totalItems,
    ...
    useEffect(() => {
        const _shapedIndex = (index_.value = Math.min(Math.max(activeIndex, 0), totalItems - 1))
        if (onIndexChange) {
